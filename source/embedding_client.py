"""
Embedding and reranker client for content-analyser.
...
class EmbeddingClient:
    ...
    def test_reranker(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict with ``reranker_connected`` and ``reranker_model``.

        Raises:
            EmbeddingError: If the reranker is unreachable.
        """
        if not self._config.reranker_base_url:
            raise EmbeddingError("No reranker URL configured")
        model = self.reranker_model
        return {"reranker_connected": True, "reranker_model": model}
